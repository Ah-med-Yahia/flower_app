...
@GenerateMocks([RemoteOrdersDataSourceImpl])
void main() {
  ...
  group('get user orders repo implementaion test cases', () {
    _testGetUserOrdersSuccessCase(
      mockRemoteOrdersDataSourceImpl,
      ordersRepoImpl,
    );
    _testGetUserOrdersEmptyListCase(
      mockRemoteOrdersDataSourceImpl,
      ordersRepoImpl,
    );
    ...
    _testGetUserOrdersFailureCaseWithDioException(
      mockRemoteOrdersDataSourceImpl,
      ordersRepoImpl,
    );
  });
...
void _testGetUserOrdersSuccessCase(
  ...
  test('should return ', () async {
    ...
    when(
      mockRemoteOrdersDataSourceImpl.getUserOrders(),
    ).thenAnswer((_) async => BaseResponse.success(mockResponse));

    final result = await ordersRepoImpl.getUserOrders();
    final success = result as Success<OrdersResponseEntity>;
    expect(result, isA<OrdersResponseEntity>());
    expect(success.data.orders, mockResponse.orders);
    expect(mockResponse.toEntity().orders!.length, success.data.orders!.length);
    verify(mockRemoteOrdersDataSourceImpl.getUserOrders()).called(1);
  });
}

void _testGetUserOrdersEmptyListCase(
  MockRemoteOrdersDataSourceImpl mockRemoteOrdersDataSourceImpl,
  OrdersRepoImpl ordersRepoImpl,
) {
  test('should return success with empty orders list', () async {
    final OrdersResponse mockResponse = OrdersResponse(
      message: 'Orders fetched successfully',
      metadata: OrdersMetadata(
        currentPage: 1,
    ...
    expect(result, isA<OrdersResponseEntity>());
    expect(success.data.orders, isEmpty);
    expect(mockResponse.toEntity().orders!.length, success.data.orders!.length);
...
void _testGetUserOrdersFailureCase(
  ...
  test('should return error when get user orders fails', () async {
    ...
    expect(failure, isA<Failure<OrdersResponseEntity>>());
...
void _testGetUserOrdersFailureCaseWithDioException(
  MockRemoteOrdersDataSourceImpl mockRemoteOrdersDataSourceImpl,
  OrdersRepoImpl ordersRepoImpl,
) {
  test(
    ...
    () async {
      ...
      final handelError = ErrorHandler.handle(dummyError);
      when(
        mockRemoteOrdersDataSourceImpl.getUserOrders(),
      ).thenAnswer((_) async => BaseResponse.failure(handelError));
      final result = await ordersRepoImpl.getUserOrders();
      final failure = result as Failure<OrdersResponseEntity>;
      expect(failure, isA<Failure<OrdersResponseEntity>>());
      expect(failure.errorHandler, equals(handelError));
      expect(
        failure.errorHandler.errorModel.message,
        ErrorsConstant.noInternetError,
      );
      expect(failure.errorHandler.errorModel.code, handelError.errorModel.code);
      expect(
        failure.errorHandler.errorModel.message,
        handelError.errorModel.message,
      );
      verify(mockRemoteOrdersDataSourceImpl.getUserOrders()).called(1);
    },
  );
