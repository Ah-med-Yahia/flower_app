...
import 'package:flower_app/features/cart/data/datasources/cart_remote_data_source.dart';
...
void main() {
  ...
  void verifyOnlyThisCall(Function verification, dynamic mockObject) {
    verification(); // Execute the verification
  ...
  setUp(() {
    reset(mockCartApiClient);
  });

  group('getCart', () {
    test(
      'should return GetCartResponseModel when the API call is successful',
      () async {
        when(
        ...
        expect(result, isA<GetCartResponseModel>());
    ...
    test('should throw an exception when the API call fails', () async {
      ...
      expect(() async => await cartRemoteDataSource.getCart(), throwsException);
      verifyOnlyThisCall(
        () => verify(mockCartApiClient.getCart()).called(1),
        mockCartApiClient,
      );
  ...
  group('addToCart', () {
    ...
    test('should throw an exception when the API call fails', () async {
      ...
      expect(
        () async => await cartRemoteDataSource.addToCart(
          requestModel: tAddToCartRequestModel,
        ),
        throwsException,
      );
  ...
  group('updateItemQuantity', () {
    ...
    test('should throw an exception when the API call fails', () async {
      ...
      expect(
        () async => await cartRemoteDataSource.updateItemQuantity(
          productId: tProductId,
          requestModel: tUpdateItemQuantityRequestModel,
        ),
        throwsException,
  ...
  group('removeItem', () {
    ...
    test('should throw an exception when the API call fails', () async {
      ...
      expect(
        () async => await cartRemoteDataSource.removeItemFromCart(
          productId: tProductId,
        ),
        throwsException,
      );
  ...
  group('clearCart', () {
    test(
      'should return ClearCartResponseModel when the API call is successful',
      () async {
        when(
          mockCartApiClient.clearCart(),
        ).thenAnswer((_) async => tClearCartResponseModel);

        final result = await cartRemoteDataSource.clearCart();

        expect(result, isA<ClearCartResponseModel>());
