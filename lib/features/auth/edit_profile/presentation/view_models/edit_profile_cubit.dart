...
class EditProfileCubit extends Cubit<EditProfileStates> {
  ...
  EditProfileCubit(this._updateUserProfileUseCase, this._uploadImageUseCase)
  ...
  Future<void> _uploadImage(String imagePath) async {
    emit(
      state.copyWith(
        imageUploadState: const BaseState<MessageResponse>(isLoading: true),
      ),
    );

    final res = await _uploadImageUseCase.execute(imagePath);
    res.when(
      success: (data) => emit(
        state.copyWith(
          imageUploadState: BaseState<MessageResponse>(
            isLoading: false,
            data: data,
            errorMessage: null,
          ),
