...
class SavedAddressesCubit extends Cubit<SavedAddressesStates> {
  ...
  void onEvent(SavedAddressesEvent event) {
    switch (event) {
      ...
      case EditSavedAddressEvent(:final addressId):
        _navigateToAddUpdateAddressScreen(addressId);
  ...
  void _navigateToAddUpdateAddressScreen(String? addressId) {
    emit(
      state.copyWith(
        navigationEvent: NavigateToAddUpdateAddressUiEvent(addressId),
      ),
    );
  }
}
